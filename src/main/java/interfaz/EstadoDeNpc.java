...
public class EstadoDeNpc {
	...
	/*
	 * if(npc.getEnergia() == npc.getEnergiaTope()) { drawBarra =
	 * ANCHOBARRA; } else { drawBarra = (npc.getEnergia() * ANCHOBARRA) /
	 * npc.getEnergiaTope(); }
	 * 
	 * g.drawImage(Recursos.barraEnergia, x + 80, y + 42, drawBarra,
	 * ALTOENERGIA, null); g.drawString(String.valueOf(npc.getEnergia()) +
     ...
     * ALTOMINIATURA, null);
     * 
     * g.setColor(Color.WHITE); g.setFont(new Font("Tahoma", Font.PLAIN, 10));
     * g.drawImage(Recursos.barraSalud, x + 80, y + 26, ANCHOBARRA, ALTOSALUD,
     * null); g.drawString(String.valueOf(personaje.getSaludTope()) + " / " +
